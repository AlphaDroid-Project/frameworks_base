...
internal class AxQsEditListState<T>(initialItems: List<AxQsGridItem<T>>) {
    ...
    // Returns false if [id] is gone (repacked away) — caller must not startTransfer.
    // Aborted platform transfers leave dragInProgress stuck (no DRAG_ENDED); recover here on
    // the next long-press instead of check()-crashing SystemUI.
    fun start(id: String, anchor: Offset): Boolean {
    ...
    return dragAndDropSource(
        block = {
            detectDragGesturesAfterLongPress(
                onDrag = { _, _ -> },
                onDragStart = start@{ anchor ->
                    // start() recovers a stale session from an aborted transfer.
                    // Do not wire onDragCancel = cancel: a successful startTransfer cancels the
                    // pointer into the platform drag, and clearing draggedId there would break
                    // move/drop (stock QS leaves cancel as a no-op for the same reason).
                    // onDragEnd only runs when the gesture ends before a session starts.
                    if (!state.start(id, anchor)) return@start
                    currentOnDragStart()
                    startTransfer(
